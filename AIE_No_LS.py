...
def intensityOptLoss(firstDoC, intermediateDoC, finalDoC, target): #as per MSEC
    ...
    intermediateLoss = torch.linalg.matrix_norm(intermediateDoC - 0.30 * target,'fro')
...
dfsvty=float(20e-10) #m2^2/s
#dfsvty=float(200e-12) #O2 concentration-dependent
TEMPO_dfsvty=float(40e-11) #m2^2/s, TEMPO diffusion coefficient
...
#0.2 for 5fps
total_steps=int(15/dt)
tstepT0 = int(2.5 / dt) # only for loss and optimization.
tstepT1 = int(5.1 / dt) # When epoch is 1 for the simulation, Loss does not matter
tstepT2 = int(7.7 / dt) # But need to change with DoC profile with distinct intensity
#O2_inhibition_time=1.632
...
Totalinhibtion=81.323
#0 for no inhibition
#22.665 for 1mmol TEMPO concentration
#81.323 for 5mmol TEMPO concentration
...
for epoch in range(numEpochs):
    ...
    blur_mask=opt_mask # for no scattering, just for testing
    ...
    A = 0.0358*(blur_mask.clamp(min=1e-12)/255 * intensity) + 0.1450
    ...
    for step in range(total_steps):
        ...
        #For local TEMPO with no diffusion
        #TEMPO_diffused=TEMPO[-1]
    ...
    Loss=intensityOptLoss(DoC[tstepT0], DoC[tstepT1], DoC[tstepT2], target=(mask/255)).to(device)  # mask is [0,255], /255 gives [0,1] DoC target

...
folder_name = 'test_repro_fine_grid'
...
for epoch in range(numEpochs):
    ...
    Loss=intensityOptLoss(DoC[tstepT0], DoC[tstepT1], DoC[tstepT2], target=(target_mask/255)).to(device)
    #SML=ssim_loss((DoC[-1]>(15/255)).view(1,1,H,W), ((target_mask/255)>(15/255)).view(1,1,H,W),data_range=1.0).to(device)
    #Loss=1-SML
    if epoch % 100 == 0:
...
final_fine_mask = expand_projector_mask(opt_mask, REFINEMENT)
blur_mask_pre=final_fine_mask.unsqueeze(0).unsqueeze(0)
